...
begin
  ...
  begin
    ...
    FOnProgress(Self, Item[FProcessedItemIndex].FileName,
      CurrentProgress, TotalProgress, Cancel, ProgressState);
...
procedure TFWZipWriter.SaveItemToStream(Stream: TStream; Index: Integer);
  ...
  function CreateCompressor(Level: TCompressionLevel; Stream: TStream): TCompressionStream;
  begin
    {$IFDEF USE_ZLIB_EX}
    Result := TCompressionStream.Create(Stream, Level);
    {$ELSE}
    Result := TCompressionStream.Create(Level, Stream);
    {$ENDIF}
  end;
...
begin
  ...
  if (CurrentItem.FilePath = '') and (CurrentItem.Data = nil) then
    raise EZipWriter.Create('Данные элемента отсутствуют');
