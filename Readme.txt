FWZip версия 2.0.15 (от 25 сентября 2026 года)
...
2. Список изменений:
  ...
  2.0.15 (от 25 сентября 2026 года):
    - Исправлена критическая ошибка с задваиванием размера ExData в TFWZipWriter работающего через TFWZipModifier при наличии в архиве файлов без атрибутов, что приводило к созданию разрушеного архива.
	
  2.0.14 (от 22 сентября 2026 года):
    - Добавлена поддержка NTFS аттрибутов для папок. Хоть стандарт это и не оговаривает отдельно, но не запрещает, поэтому сразу добавлена поддержка для архивов созданых сторонними архиваторами.

  2.0.13 (выпуск не осуществлялся)
    - Исправлено отсутствие коррекции FileCommentLength в TFWZipModifier, которое могло привести к разрушению архива при изменении коментария.

  2.0.12 (от 11 сентября 2026 года):
    - Добавлены апи работы со словарем при сжатии/распаковке в FPC (deflateSetDictionary/inflateSetDictionary)
    - TFWZipModifier теперь поддерживает изменение комментариев элементов (спасибо Алексею Хорту за найденую ошибку).

  2.0.11 (от 1 августа 2026 года):
    - Добавлено событие TFWZipReader.OnExtractFilter для задания комбинированного фильтра при распаковке архива.
    - Расширен пример ExctractZIPDemo1 для демонстрации работы с событием OnExtractFilter
    - В FWZipUtils добавлены в интерфейсную секцию SystemTimeToFileTime и FileTimeToSystemTime для Linux

  2.0.10 (от 7 сентября 2025 года):
    - Исправлена критическая ошибка при создании многотомного архива. В некоторых ситуациях мог увеличиваться номер тома, без фактического создания нового тома.
    - Добавлены следующие изменения за авторством Danil Zaripov (https://github.com/DonilZ):
    - Добавлена совместимость с Linux при сборке из Delphi 
    - Добавлен проект юнит-тестирования для Delphi 2007
    - Добавлен утилитарный модуль для поддержки режима Delphi + Linux

  2.0.9 (от 17 июня 2025 года):
    - Реализованы предложения от Алексея Хорта:
    - Введен контроль слэша в передаваемой параметром маске в процедуре ProcessExtractOrCheckAllData.
    - Добавлен перекрытый метод TFWZipReader.Find позволяющий продолжать поиск на дублирующихся именах элементов.
    - Добавлен метод TFWZipReader.FindByMask поволяющий производить поиск по маске.
    - Исправлена ошибка в рассчетах прогресса при сжатии файлов с нулевым размером.	

  2.0.8 (от 25 мая 2025 года):
    - Реализованы предложения от Алексея Хорта:
    - Расширен метод InsertStream, добавлен перекрытый метод позволяющий указать атрибуты файла.
  ...
  2.0.7 (от 9 марта 2025 года):
    ...
    - Возвращена поддержка Delphi 7 сломанная неверно объявленным дефайном.	

  2.0.6 (от 14 января 2025 года):
    - Введен новый тип обработки исключений TExceptionAction.eaReRaise и глобальная настройка DefaultExceptionAction. 
...
3. Описание файлов:
  ...
  - .\FWZipLinuxDelphiCompability.pas - Совместимость библиотеки FWZip с Linux+Delphi (автор Danil Zaripov - github.com/DonilZ)
  ...
  - .\.UnitTest\* - папка с проектами юнит тестов всех классов фреймворка
