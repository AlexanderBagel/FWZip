FWZip версия 1.0.10 от ?? ???? 2013 года.
...
2. Список изменений:
  ...
  1.0.7 (от 10 июня 2013 года)
    - исправлена критическая ошибка в модуле FWZipReader приводящая к разрушению памяти приложения при откртии специальным образом сформированных файлов.
  ...
  1.0.10 (от ?? ???? 2013 года)
	...
	- Во всех исключениях модуля FWZipReader предоставляется более подробная информация об ошибке.
	Реализован набор предложений от Владислава Нечепоренко:	
	...
	- для возможности работы с элементом архива до того момента пока он не залочен, добавлены два состояния прогреса psStart и psEnd (см. тип TProgressState). 
	  ВАЖНО!!!
	  1: Метод TFWZipReaderItem.ExtractToStream данные состояния не генерирует.
	  2: При вызове метода TFWZipReader.Check генерация этих состояний не происходит (т.к. файлы реально не создаются)
...
3. Описание файлов:
  ...
  - .\Demos\ZipAnalizer\ - папка с проектом анализатора ZIP архивов
  ...
  - .\error\error.zip - пример архива созданного архиватором 7Zip, который не может быть распакован при помощи FWZip (см. описание в пункте 12)
...
12. Известные ошибки - FWZip не всегда может корректно извлечь данные в том случае если архив был создан архиватором 7Zip. 
Происходит ошибка распаковки данных внутри ZLib (или ZLibEx).
Пример такого архива в папке "error", в нем находится картинка, которая при распаковке через TDecompressionStream извлекается не полностью (последние 54 байта отсутствуют). Тут пока что ничего поделать нельзя, до тех пор пока не выйдет ZLib совместимый с 7Zip deflate.