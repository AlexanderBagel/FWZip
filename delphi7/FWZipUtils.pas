...
//  * Purpose   : Ќабор платформозависимых методов
...
{$IFDEF FPC}
  {$MODE Delphi}
  {$H+}
  // у FPC криво реализованы инлайны, в частности FileSizeToInt64 тер€ет
  // значение FileSizeHi при компил€ции в 32 бита, поэтому отключаем
  {$IFDEF CPU32}
    {$UNDEF USE_INLINE}
  {$ELSE}
    {$DEFINE USE_INLINE}
...
uses
  ...
  // утилитарные преобразовани€ дл€ отключени€ ворнингов под FPC
  // ===========================================================================
  ...
  // это позволит избежать сообщени€ об неинициализированных переменных под FPC
...
// эмул€ци€ апи дл€ работы с NTFS атрибутами, в частности с временем
...
const
  ...
  MINSPERHOUR = 60;
  HOURSPERDAY = 24;
  EPOCHWEEKDAY = 1;  // 1 €нвар€ 1601 был понедельником
  DAYSPERWEEK = 7;
...
begin
  ...
  // если текущий мес€ц меньше марта, то просто отнимаем год и добавл€ем 12 мес€цев
  // это учтетс€ в дальнейшем
  CalcYear := ATime.Year;
  ...
  // количество високосных годов в пределах столети€
  ...
  CalcDay :=
    ...
    (1959 * (CalcMonth + 1)) shr 6 +             // мес€ц * среднее кол-во дней
    ATime.Day -                                  // день
...
begin
  ...
  // вытаскиваем количество миллисекунд и преобразуем врем€ в секунды
  ...
  // считаем год, мес€ц и день мес€ца
  CalcLeapCount :=
  ...
  // результат дл€ года который начинаетс€ с марта, если залазит на следущий
  // то дл€ преобразовани€ отнимаем 12 мес€цев и увеличиваем год
...
begin
  ...
  {$ELSE}
  // Rouse_ 25.10.2013
  // ѕравка небольшой ошибки замеченой ¬ладиславом Ќечепоренко
