...
begin
  ...
  try
    ...
    try
      ...
      // Еще одним методом работы с перезаписываемыми элементами
      // является работа с флагом DefaultDuplicateAction
      // Например если часто обновляется один и тот-же архив, а выгружать
      ...
      // специально для теста дату последней записи FWZipModifier на диске
      // поменяем на несколько недель назад
      ...
      // в результате у элемента FWZipModifier.pas должен стоять результат erDone
      Writeln;
