  ...
  try
    ...
    case FFileHeader.Header.CompressionMethod of
      ...
      begin
        ...
        try
          // Rouse_ 30.10.2013
          // Раньше использовался автогенерируемый заголовок ZLib
          // При использовании windowBits заголовок не требуется
          // Это учитывается в классе TFWZipItemStream
          {$IFDEF USE_AUTOGENERATED_ZLIB_HEADER}
          Decompressor := TDecompressionStream.Create(ZipItemStream);
          {$ELSE}
          Decompressor := TDecompressionStream.Create(ZipItemStream, -15);
          {$ENDIF}
          try
